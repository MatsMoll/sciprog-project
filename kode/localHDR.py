...
def check_dim(im):
    """
    Bool function that checks if there is a alpha channel (dimension) present. [RENAME FUNC!]
...
def edit_blurred_image(blurry_im, lum_scale=1, chrom_scale=1, effect=.1, func="pow", mode="global"):
    """
...
def filter_image(im, linear=True, sigma=3, mode="global", lum_scale=10, chrom_scale=.3, gamma=5, effect=1, func="pow"):
    ...
    blurry_im_edited = edit_blurred_image(blurry_im, lum_scale, chrom_scale, effect, func, mode)
